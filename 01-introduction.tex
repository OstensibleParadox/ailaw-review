...
Technical AI research has successfully translated humanistic concepts into engineering specifications---attention became transformers, preference became reward models---but no equivalent reverse translation has given systems engineering the vocabulary to diagnose emergent hazards that originate deep within the control architecture and produce catastrophic unmodeled dynamics. This paper supplies one limitative result that STPA cannot derive from its own vocabulary: a feedback non-recoverability bound on trace-only control. It then translates that limit into STPA terms so the bound can discipline engineering practice.

We introduce the Constraint Cascade (hereafter, the Cascade), a four-layer diagnostic taxonomy developed as an engineering-facing interface for that bound. The Cascade partitions LLM deployment by diagnostic role: Observable Environment (L1), Control Blind Spot (L2), Governance-Object Vacuum (L3), and Authority Control (L4), with an epistemic boundary between L2 and L3 formalized as a feedback non-recoverability corollary from companion work~\cite{anon2026dual}. The argument first states that constraint and its control-theoretic meaning (\S\ref{sec:cascade}); then it translates the bound into STPA-facing audit terms---losses, hazards, system constraints, Unsafe Control Actions, and loss-scenario classes---so engineers can see where the loop fractures (\S\ref{sec:patterns}). The resulting control deficit requires architectural redundancy and forced state disclosure rather than output-only component patches (\S\ref{sec:responsibility}). Five cases span the range from no human agency to full deployer intent, the last prospective---a licensed clinical gatekeeper whose epistemically correct refusal the architecture cannot discharge (\S\ref{sec:cases}).
