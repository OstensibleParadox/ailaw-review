...
\centering
\small
\caption{The four realized-harm cases mapped through STPA interface vocabulary: unsafe control action (Table~\ref{tab:uca}), loss-scenario class (\S\ref{sec:patterns}), and hazard with violated constraint (Table~\ref{tab:hazards}). Case~(e) is prospective (harm averted by refusal) and analyzed in text.}
\label{tab:cases}
...
\midrule
(d) Yuanbao & Structural (none) & CA2 not provided / provided: no modality classifier; filter masks persisting disposition & II: inadequate process model --- contamination (train-time) & H-2 $\to$ SC-2; L2/L3 \\
\bottomrule
