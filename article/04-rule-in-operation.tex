...
Two allocations should be stated expressly. First, the employer's own configuration is itself a control lane: the screening threshold is specifically connected to the asserted disparity, configuring is among the abilities Model Act \S~5(b) names as practical authority, and both sit on the event path---a downstream actor that selects the consequential settings ``can occupy the relevant lane instead.'' Visibility assigns the employer the first-answer duty; it does not assign an exit from Stage Two. Second, the custodian map is the upstream supplier's defense asset: the customer's own first answer identifies configuration authority and custodianship, produced by the customer at the customer's cost before any merits fight. Part IV.F calls the record a defense asset; on a divided stack it is specifically the upstream party's.
